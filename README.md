...
- `目录!M1:O8` 固定为“表头 + 汇总 + 最新五周 + 归档”；五周按上旧下新排列，N2:O8 每次按实际 Sheet ID 重建并校验链接，不向第 9 行以后追加。
- 慢 SQL 长文本自适应分块读取，并以 base64 写入；单行无法完整读取时停止。
- 两个归档页全部回读校验成功后，才删除对应源 Sheet。
- 确认源 Sheet 消失后，重建并回读校验固定目录区；`M1:O8` 之外保持不变。
- 命令可重跑：一致的周期块不会重复追加；中途删除失败可依据双归档块和残留单边源 Sheet 恢复，不依赖历史目录行。
